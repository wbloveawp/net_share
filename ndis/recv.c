...
//申请发送包
PNDIS_PACKET ndisprotAllocateSendPacket(IN PNDISPROT_OPEN_CONTEXT pOpenContext, IN UINT DataLength, OUT PUCHAR*  ppDataBuffer)
{
...
//释放发送包
VOID ndisprotFreeSendPacket(IN PNDISPROT_OPEN_CONTEXT pOpenContext, IN PNDIS_PACKET pNdisPacket)
{
	PNDIS_BUFFER        pNdisBuffer;
	UINT                TotalLength;
	UINT                BufferLength;
	...
	if (NdisGetPoolFromPacket(pNdisPacket) == pOpenContext->SendPacketPool)
	{
		//
...
//转发ssl包
/*
VOID MySendSSLPack(IN PNDISPROT_OPEN_CONTEXT pOpenContext, IN PNDIS_PACKET pNdisPacket)
{
	return VOID();
	
	PNDIS_PACKET pCopyPacket_Send;

	//1.申请一个sendpack
	pCopyPacket_Send = ndisprotAllocateSendPacket(pOpenContext, TotalPacketLength, &PackData);
	//2.数据拷贝
	while (1)
	{
		NdisQueryBufferSafe(pNdisBuffer, &tmp_buf, &copysize, NormalPagePriority);
		NdisMoveMemory(PackData + offset, tmp_buf, copysize);
		offset += copysize;
		NdisGetNextBuffer(pNdisBuffer, &pNdisBuffer);
		if (pNdisBuffer == NULL)
			break;
	...
	//3.发送pack
	DbgPrint("端口为443,直接写入网卡\n");
	NPROT_REF_OPEN(pOpenContext);
	NdisSendPackets(pOpenContext->BindingHandle, &pCopyPacket_Send, 1);
	DbgPrint("NdisSendPackets发送ok\n");
	return XY_PACK_IS_SSL;
	
}*/

//自定义初步过滤 返回TRUE则丢弃  FALSE则继续,其次判断是否为ip包
//判断此包是否来自本网段
UINT MyFilterPro(
	IN NDIS_HANDLE                  ProtocolBindingContext,
    IN PNDIS_PACKET                 Packet)
{
	...
	NDIS_STATUS         Status ;
	//UINT pc, bc;
	//UINT copysize, offset = 0;
	//PUCHAR tmp_buf;
	//PUCHAR			PackData;
	UINT BufferLength, TotalPacketLength;
	PNDIS_BUFFER pNdisBuffer ;
	PETH_PACK mac_h ;
	...
	NdisGetFirstBufferFromPacketSafe(
		...
		NormalPagePriority);
#else
	NdisQueryPacket(Packet,&pc,&bc,&pNdisBuffer,&TotalPacketLength) ;
	NPROT_ALLOC_MEM(PackData, TotalPacketLength) ;
	NdisZeroMemory(PackData, TotalPacketLength) ;
	while(1)
	{
		NdisQueryBufferSafe( pNdisBuffer, &tmp_buf,  &copysize, NormalPagePriority ) ; 
		NdisMoveMemory(PackData+offset,tmp_buf,copysize) ;
	...
	mac_h = (PETH_PACKET)PackData;
#endif
	ret = PACK_NO_NEED;
	if (TotalPacketLength < sizeof(NDISPROT_ETH_HEADER) + sizeof(IP_HEADER))
		return ret;
	do
	{
		IP_HEADER *ip_h ;
		if(mac_h->EthType !=IP_FLAG)
		{
			ret = PACK_NO_IP ;
			break ;
		}
		...
		if(ip_h->cTypeOfProtocol!=PRO_ICMP && ip_h->cTypeOfProtocol!=PRO_TCP && ip_h->cTypeOfProtocol!=PRO_UDP)
		{
			ret = PACK_NO_UDP_TCP_ICMP ;
			break ;
		}
		//增加判断，端口是否为443
#if 0
		if (ip_h->cTypeOfProtocol == XY_PRO_TCP)
		{
			short ssl_port;
			ssl_port = SSL_PORT;
			if (ssl_port == mac_h->tcp_h.sSourPort || ssl_port == mac_h->tcp_h.sDestPort)
			{
				MySendSSLPack(pOpenContext, Packet);
			}
		}
#endif
		//判断是否为本网段数据包
		if((ip_h->uiDestIp & Globals.net_ip_filter->uiNetMasks )==Globals.net_ip_filter->uiNetSeccion)
		{
			ret = PACK_IN_LAN ;//本网段TCP、UDP、ICMP包放过
			break ;
		}
		if(Globals.net_ip_filter->cFlag)//mac过滤
		{
			UINT i;
			for(i=0;i<Globals.net_ip_filter->iCount;i++)
			{
				if((mac_h->dstMac&0x00FFFFFF)==Globals.net_ip_filter->uiMacSeccion[i])
				{
					ret = PACK_IS_NEED ;
					break ;
				}	
		...
		{
			ret = (ip_h->uiSourIp & Globals.net_ip_filter->uiNetMasks )==Globals.net_ip_filter->uiNetSeccion?PACK_IS_NEED:PACK_NO_NEED ;
		}
	}while(0) ;
	//ret=0 ; //ARP包，放过,这个即不拦截，也不入队
	//NdisFreeMemory(PackData,len ,(ULONG)("WBXY")) ;
	return ret ;
}
...
--*/
{
    ...
    do
    {
            ...
            (pOpenContext->PowerState == NetDeviceStateD0))
        {
			//只需要再此初步过滤即可
			//DbgPrint("开始过滤..\n") ;
			if(MyFilterPro(pOpenContext,pRcvPacket)== PACK_IS_NEED)
			{
				//DbgPrint("MyFilterPro 入队操作..\n") ;
        ...
        if (pOpenContext->RecvPktCount > MAX_RECV_QUEUE_SIZE)
        {
            ...
            //
			DbgPrint("此时队列包过多[%ud][%d]，清除队头数据包----------->\n", pOpenContext->RecvPktCount, MAX_RECV_QUEUE_SIZE);
