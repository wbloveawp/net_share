...
#define NT_DEVICE_NAME          L"\\Device\\wbNdis"
#define DOS_DEVICE_NAME         L"\\DosDevices\\wbNdis"
...
typedef struct _NDISPROT_OPEN_CONTEXT
{
    ...
    ULONG                   MacOptions;
    ULONG                   MaxFrameSize;   //网卡传输帧最大值，IP包要小于等于此值
...
typedef struct _NDISPROT_GLOBALS
{
	...
	//WB:新增过滤网段
	int						net_ip_filter_len;
	NET_IP_FILTER*			net_ip_filter ;
...
//1:非ICMP，TCP，UDP包
//2:本网段包
//3:非本网段包  当返回为3时，才入队，并丢弃包，其他放过也不入队
UINT MyFilterPro(
	IN NDIS_HANDLE                  ProtocolBindingContext,
    IN PNDIS_PACKET                 pNdisPacket) ;
//WB:获取网卡列表
