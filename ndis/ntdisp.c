...
--*/
{
	//char * pro_name = XUNYOU_NDIS_PROTO_NAME;
    NDIS_PROTOCOL_CHARACTERISTICS   protocolChar;
    NTSTATUS                        status = STATUS_SUCCESS;
    NDIS_STRING                     protoName = NDIS_STRING_CONST("wbNdis");
	...
	//WB：打印我需要的信息
	DbgPrint("IOCTL_NDISPROT_OPEN_DEVICE=0x%x\n",IOCTL_NDISPROT_OPEN_DEVICE) ;
	DbgPrint("IOCTL_NDISPROT_QUERY_OID_VALUE=0x%x\n",IOCTL_NDISPROT_QUERY_OID_VALUE) ;
	DbgPrint("IOCTL_NDISPROT_SET_OID_VALUE=0x%x\n",IOCTL_NDISPROT_SET_OID_VALUE) ;
	DbgPrint("IOCTL_NDISPROT_QUERY_BINDING=0x%x\n",IOCTL_NDISPROT_QUERY_BINDING) ;
	DbgPrint("IOCTL_NDISPROT_BIND_WAIT=0x%x\n",IOCTL_NDISPROT_BIND_WAIT) ;
	DbgPrint("IOCTL_NDISPROT_INDICATE_STATUS=0x%x\n",IOCTL_NDISPROT_INDICATE_STATUS) ;
	DbgPrint("IOCTL_NDISPROT_GET_DEVICE_LIST=0x%x\n",IOCTL_NDISPROT_GET_DEVICE_LIST) ;
	DbgPrint("IOCTL_NDISPROT_SET_HOST_SECCION=0x%x\n",IOCTL_NDISPROT_SET_HOST_SECCION) ;
	DbgPrint("IOCTL_NDISPROT_GET_NETWORK_BASE_INFO=0x%x\n",IOCTL_NDISPROT_GET_NETWORK_BASE_INFO) ;
	DbgPrint("IOCTL_NDISPROT_GET_NETWORK_TRANS_INFO=0x%x\n",IOCTL_NDISPROT_GET_NETWORK_TRANS_INFO) ;
	DbgPrint("IOCTL_NDISPROT_CLOSE_DEVICE=0x%x\n",IOCTL_NDISPROT_CLOSE_DEVICE) ;
...
--*/
{
    ...
    switch (FunctionCode)
    {
        ...
        case IOCTL_NDISPROT_INDICATE_STATUS:
            ...
            break;/*
		case IOCTL_NDISPROT_GET_DEVICE_LIST:
			{
				//WB:获取网卡列表//WB:获取网卡列表
				//填写到此缓冲区pIrp->AssociatedIrp.SystemBuffer相对于DeviceIoControl的outbuffer
				PNDISPROT_OPEN_CONTEXT pAdapter ;
				pAdapter = GetDeviceList() ; 
				if(pAdapter!=NULL)
				{
					//判断缓冲区大小
					if(pIrpSp->Parameters.DeviceIoControl.OutputBufferLength<pAdapter->DeviceName.Length)
					{
						//缓冲区不够大
						NtStatus = NDIS_STATUS_BUFFER_OVERFLOW ;//存储空间不足，无法处理此命令
						BytesReturned = pAdapter->DeviceName.Length ;//告诉上层需要的字节数
					}
					else
					{	//pIrp->UserBuffer 输出缓冲区
						memcpy(pIrp->AssociatedIrp.SystemBuffer,pAdapter->DeviceName.Buffer,pAdapter->DeviceName.Length) ;
						BytesReturned = pAdapter->DeviceName.Length ;
						NtStatus = STATUS_SUCCESS;
					}
				}
				else
				{
					BytesReturned = 0 ;//没有找到网卡
					NtStatus = STATUS_SUCCESS;	
				}
				DbgPrint("获取网卡列表:%s字节数：%d\n",pIrp->AssociatedIrp.SystemBuffer,BytesReturned);
			}
			break ;*/
        case IOCTL_NDISPROT_SET_HOST_SECCION:
			{
				//WB:设置过滤网段 指定网卡
				//PNDISPROT_OPEN_CONTEXT p ;
				//p= GetDeviceList() ;
				//GetDeviceList() ;
				if(pIrpSp->Parameters.DeviceIoControl.InputBufferLength>=sizeof(NET_IP_FILTER))
				{
					PNET_IP_FILTER nif = (PNET_IP_FILTER)pIrp->AssociatedIrp.SystemBuffer ;
					
					NdisAllocateMemoryWithTag((PVOID*)&Globals.net_ip_filter, pIrpSp->Parameters.DeviceIoControl.InputBufferLength, 0);

					if (Globals.net_ip_filter)
					{
						memcpy(Globals.net_ip_filter, nif, pIrpSp->Parameters.DeviceIoControl.InputBufferLength);
						Globals.net_ip_filter_len = pIrpSp->Parameters.DeviceIoControl.InputBufferLength;
						NtStatus = STATUS_SUCCESS;
						DbgPrint("成功设置过滤条件:模式[%s] 网关=%d  IP=%d  \n", Globals.net_ip_filter->cFlag==0x0?"IP":"MAC",
							Globals.net_ip_filter->uiNetMasks, Globals.net_ip_filter->uiNetSeccion);
					}
				}
				else
					NtStatus = NDIS_STATUS_BUFFER_OVERFLOW ;
			}break ;
		case IOCTL_NDISPROT_GET_NETWORK_BASE_INFO:
			{
				//WB：获取网卡基本信息
				PNDISPROT_OPEN_CONTEXT pAdapter ;
				//取得网卡名
				//
				pAdapter = ndisprotLookupDevice(
				...
				{
					//判断缓冲区大小
					if(pIrpSp->Parameters.DeviceIoControl.OutputBufferLength<sizeof(BASE_NET_INFO))
					{
						NtStatus = NDIS_STATUS_BUFFER_OVERFLOW ;//存储空间不足，无法处理此命令
						BytesReturned = pAdapter->DeviceName.Length ;//告诉上层需要的字节数
					}
					else
					{
						...
						PBASE_NET_INFO bni= (PBASE_NET_INFO)pIrp->AssociatedIrp.SystemBuffer;
						bni->ulMaxFrameSize = pAdapter->MaxFrameSize ;
						memcpy(bni->szCurrentAddress,pAdapter->CurrentAddress,NPROT_MAC_ADDR_LEN) ;
						memcpy(bni->szDeviceDescr,pAdapter->DeviceDescr.Buffer,pAdapter->DeviceDescr.Length) ;
						BytesReturned = sizeof(BASE_NET_INFO) ;
						NtStatus = STATUS_SUCCESS;
						DbgPrint("网卡基本信息:帧[%d] mac[%s] 描述[%s]\n",bni->ulMaxFrameSize,bni->szCurrentAddress,bni->szDeviceDescr);
					}
				}
				else
				{
		...
		case IOCTL_NDISPROT_CLOSE_DEVICE:
			{
				//WB：关闭网卡
				...
				if(pAdapter!=NULL)
				{
					ndisprotShutdownBinding(pAdapter) ;
...
#endif // DBG
//WB:获取网卡列表
PNDISPROT_OPEN_CONTEXT GetDeviceList()
{
         ...
         pEnt = pEnt->Flink)
    {
        ...
        //
		DbgPrint("网卡:test") ;
		DbgPrint("网卡:%d %s \n",pOpenContext->DeviceName.Length,pOpenContext->DeviceName.Buffer) ;
    }
    ...
    pOpenContext = NULL;
    NPROT_ACQUIRE_LOCK(&Globals.GlobalLock);
	pOpenContext = CONTAINING_RECORD(Globals.OpenList.Flink, NDISPROT_OPEN_CONTEXT, Link);
	if(pOpenContext!=NULL)
