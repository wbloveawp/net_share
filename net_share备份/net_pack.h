...
__interface wb_link_interface
{
	...
	*/
	virtual bool OnRead(wb_filter_interface* p_fi, const ETH_PACK* pg, int len) = 0;//从网卡读取到网络包,len：包总长度
	virtual bool OnWrite(wb_filter_interface* p_fi, const ETH_PACK* pg, int len) = 0;//从网卡写完网络包
	virtual bool OnRecv(wb_filter_interface* p_fi, const char* buf, int len, LPOVERLAPPED pol) = 0;//从网卡读取到网络包,len：包总长度
	virtual bool OnSend(wb_filter_interface* p_fi, const char* buf, int len, LPOVERLAPPED pol) = 0;//从网卡写完网络包
	virtual bool OnSend_no_copy(wb_filter_interface* p_fi, char* buf, int len) = 0;//数据不进行拷贝
...
{
	virtual bool post_send(wb_link_interface* plink, const char* buf, int len) = 0;
	virtual bool post_send_no_copy(wb_link_interface* plink, char* buf, int len) = 0;//数据不进行拷贝
	virtual bool post_recv(wb_link_interface* plink, LPOVERLAPPED pol) = 0;
