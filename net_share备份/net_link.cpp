...
bool  wb_udp_link::OnRead(wb_filter_interface* p_fi, const ETH_PACK* pk, int len)
{
	_last_op_time = time(0);
	return p_fi->post_send(this, get_udp_pack_date(pk), get_udp_pack_date_len(pk));//非分片
}

bool wb_udp_link::OnRecv(wb_filter_interface* p_fi, const char* buf, int len, LPOVERLAPPED pol)
{
	...
	return p_fi->post_recv(this,pol);
}
...
wb_tcp_link::wb_tcp_link(const ETH_PACK* p_pack) :wb_net_link(p_pack) {
	...
	_ack = ntohl(p_pack->tcp_h.uiSequNum)+1;//表示我下次发送的确认序号
...
bool wb_tcp_link::OnRead(wb_filter_interface* p_fi, const ETH_PACK* pg, int len)
{
	if (_close_status)return true;
	_last_op_time = time(0);
	auto ret = true;
	//assert(_next_pid == ntohl(pg->ip_h.sPackId));
	...
	case TCP_DATA_ACK: {
		...
		ret = DoOnData(p_fi, (const char*)&pg->tcp_h + tcp_head_len, date_len, c_seq, true);
	}break; //有数据，且需要立马提交给应用层
	case TCP_ACK: ret = DoOnAck(p_fi, pg, len); break;	//确认包，也可能会携带数据,出现最频繁的，case放在最前面提高效率
...
bool wb_tcp_link::DoOnData(wb_filter_interface* p_fi, const char* pdata, int date_len,UINT c_seq, bool bPsh)
{
	assert(date_len > 0);
	//1.如果是当前的提交包，立马发送
#ifdef _DEBUG
	...
	auto now_ack = _ack;
	auto now_Psh = _psh_num;
	if (c_seq > _ack || !bPsh) //后续包或非提交包，先进行缓存
	{
	...
	else if (bPsh && c_seq == _ack)
	{
		_ack += date_len;
		if (p_fi->post_send(this, pdata, date_len))
		{
			int out_l = 0;
	...
	if (_psh_num) 
	{
		...
		while (pk && _psh_num > 0)
		{
			...
			auto fn = [&]() {
				...
				if (!p_fi->post_send(this, send_buf, send_len))
					return;
				send_len = 0;
				int out_l = 0;
				auto mk = make_pack_ack(out_l, _ack);
...
bool wb_tcp_link::DoOnAck(wb_filter_interface* p_fi, const ETH_PACK* pg, int len)
{
	//C端对S端数据进行确认或维活
	auto tcp_head_len = (pg->tcp_h.cHeaderLenAndFlag >> 4 & 0x0F) * 4;
	int date_len = ntohs(pg->ip_h.sTotalLenOfPack) - c_ip_head_len - tcp_head_len;//数据长度
	...
	{
		//if(c_seq < _ack)
			//WLOG("重发:myack=%d seq=%d dlen=%d 差距=%d \n", _ack, c_seq, date_len, _ack - c_seq);
		return true;// _ack == c_seq + date_len;//一般情况下这里返回都是tcp协议层的重发机制 
	}
	return DoOnData(p_fi, (const char*)&pg->tcp_h + tcp_head_len, date_len, c_seq, false);
}

bool wb_tcp_link::OnRecv(wb_filter_interface* p_fi, const char* buf, int len, LPOVERLAPPED pol)
{
	_last_op_time = time(0);
#ifdef _DEBUG
	//wb_auto_tm tm("OnRecv");
#endif // _DEBUG
	int dlt = 0, w_len = 0;
	int out_l = 0;
	do
	{
		auto mk = make_pack_data(buf + dlt, w_len = len > DEFAULT_RECV_SIZE ? DEFAULT_RECV_SIZE : len, out_l, len < DEFAULT_RECV_SIZE ? TCP_DATA_ACK : TCP_ACK);
		p_fi->post_write(this, mk, out_l);
		len -= w_len;
		dlt += w_len;
	} while (len>0);
